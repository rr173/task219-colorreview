...
func (s *Store) SnapshotConclusionVersion(ctx context.Context, c *model.ReviewConclusion) error {
	_, err := s.db.ExecContext(ctx,
		`INSERT INTO conclusion_versions (conclusion_id, batch_id, verdict, summary, version, snapshot_at)
		 VALUES (?, ?, ?, ?, ?, ?)`,
...
// PublishConclusion 原子地更新发布状态并写入不可变版本快照。
func (s *Store) PublishConclusion(ctx context.Context, c *model.ReviewConclusion) error {
	return WithTx(ctx, s.db, func(tx *sql.Tx) error {
		...
		if _, err := tx.ExecContext(ctx,
			`INSERT INTO conclusion_versions (conclusion_id, batch_id, verdict, summary, version, snapshot_at)
			 VALUES (?, ?, ?, ?, ?, ?)`,
			c.ID, c.BatchID, string(c.Verdict), c.Summary, c.Version, now()); err != nil {
			return fmt.Errorf("snapshot conclusion: %w", err)
		}
		return nil
	})
